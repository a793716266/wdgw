package com.flc.controller.system.subject;
...
import com.flc.service.system.subject.SubjectManager;

/**
 * 说明：项目表 创建人：王尧宇 创建时间：2018-07-25
 */
@Controller
...
public class SubjectController extends BaseController {
	...
	public void removeit(HttpSession session, HttpServletRequest request) {
		request.setAttribute("kind", "subject");
	...
	public ModelAndView save(HttpSession s, HttpServletRequest request) throws Exception {
		...
		pd.put("BIGORSMALL", request.getParameter("BIGORSMALL"));
	...
	public void delete(PrintWriter out, HttpServletRequest request) throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		PageData pageDate = subjectService.findById(pd);
		request.setAttribute("kind", "subject");
	...
	public ModelAndView list(Page page, HttpSession session, HttpServletRequest request) throws Exception {
		...
		session.setAttribute("subjectMap", maps);// maps包含查询条件如：项目名，时间，类型
		page.setPd(pd);
		...
		mv.setViewName("system/subject/subject_list");
	...
	public ModelAndView goAdd(HttpSession session) throws Exception {
		...
		mv.addObject("courses", allCourse);
		mv.setViewName("system/subject/subject_edit");
	...
	public ModelAndView exportExcel(HttpSession session) throws Exception {
		...
		Map<String, Object> maps = (Map<String, Object>) session.getAttribute("subjectMap");
		...
		for (int i = 0; i < varOList.size(); i++) {
			...
			vpd.put("var1", varOList.get(i).getString("SUBJECTNAME")); // 1
			vpd.put("var2", varOList.get(i).getString("PICTURENAME")); // 2
