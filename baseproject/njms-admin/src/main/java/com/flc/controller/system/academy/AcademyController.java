package com.flc.controller.system.academy;
...
import com.flc.service.system.academy.AcademyManager;
...
public class AcademyController extends BaseController {
	...
	String menuUrl = "academy/list.do"; //菜单地址(权限用)
	@Resource(name="academyService")
	private AcademyManager academyService;
	/**
	 * 取消
	 * @param
	 * @throws Exception
	...
	public void removeit(HttpServletRequest request){
		request.setAttribute("kind", "academy");
	...
	public ModelAndView save(HttpSession s,HttpServletRequest request) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Academy");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("ACADEMY_ID", this.get32UUID());	//主键
	...
	public void delete(PrintWriter out,HttpServletRequest request) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Academy");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		PageData pageDate=academyService.findById(pd);
		request.setAttribute("kind", "academy");
		RemoveController.deleteOne(request, pageDate.get("PICTURE").toString());
		academyService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit(HttpSession session,HttpServletRequest request) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Academy");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("PICTURE", session.getAttribute("imageName"));
		academyService.edit(pd);
		request.setAttribute("kind", "academy");
		RemoveController.saveOrUpdateRemove(request);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
	...
	public ModelAndView list(Page page, HttpSession session,HttpServletRequest request) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Academy");
		...
		session.setAttribute("academyMap", maps);//maps包含查询条件如：项目名，时间，类型
		page.setPd(pd);
		List<PageData>	varList = academyService.list(page);	//列出Academy列表
		request.setAttribute("kind", "academy");
		String path=UploadController.getPath(request);
		mv.setViewName("system/academy/academy_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("system/academy/academy_edit");
	...
	public Object deleteAll(HttpServletRequest request) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Academy");
	...
	public ModelAndView exportExcel(HttpSession session) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Academy到excel");
		...
		Map<String, Object> maps=(Map<String, Object>) session.getAttribute("academyMap");
		...
		List<PageData> varOList = academyService.listAll(pd);
